...
def print_observations(observations: List[float]):
    ...
    print("joint_positions", observations[9:21])
    print("joint_velocity", observations[21:33])
    print("last_action", observations[33:45])
...
class OnnxCommandGenerator:
    ...
    def __call__(self):
        ...
        if self.verbose:
            print("[DEBUG] raw policy output:", [f"{v:+.4f}" for v in output])
        ...
        if self.verbose:
            joint_cmds = dict(zip(ordered_joint_names_bosdyn, [f"{v:+.4f}" for v in reordered_output]))
            print(f"[DEBUG] joint pos cmd (spot order): {joint_cmds}")
            print(f"[DEBUG] ramp scale: {test_scale:.2f}, step: {self._count}")
            print("---")
